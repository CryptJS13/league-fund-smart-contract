...
contract ERC4626Wrapper is ERC20, IERC4626, ReentrancyGuard {
    ...
    constructor(IERC4626 _underlyingVault)
        ERC20(
    ...
    function _mintInternal(uint256 _shares, address _receiver, address _sender) internal returns (uint256) {
        require(_shares > 0, "Cannot mint 0");
        require(_receiver != address(0), "receiver must be defined");

        uint256 assets = previewMint(_shares);
        IERC20(asset()).safeTransferFrom(_sender, address(this), assets);
        IERC20(asset()).forceApprove(address(underlyingVault), assets);
        assets = underlyingVault.mint(_shares, address(this));
        _mint(_receiver, _shares);

        // update the contribution amount for the beneficiary
        emit IERC4626.Deposit(_sender, _receiver, assets, _shares);
        return assets;
    }

    function maxDeposit(address receiver) public view returns (uint256) {
        return underlyingVault.maxDeposit(receiver);
    }
    ...
    function deposit(uint256 _assets, address _receiver) public nonReentrant returns (uint256) {
        return _deposit(_assets, _receiver, msg.sender);
    }

    function maxMint(address receiver) public view returns (uint256) {
        return underlyingVault.maxMint(receiver);
    }

    function previewMint(uint256 _shares) public view returns (uint256) {
        return underlyingVault.previewMint(_shares);
    }

    function mint(uint256 _shares, address _receiver) public nonReentrant returns (uint256) {
        return _mintInternal(_shares, _receiver, msg.sender);
    }

    function _withdraw(uint256 _assets, address _receiver, address _owner, address _sender) internal returns (uint256) {
        require(totalSupply() > 0, "Vault has no shares");
        require(_assets > 0, "Cannot withdraw 0");

        uint256 shares = previewWithdraw(_assets);
        shares = underlyingVault.withdraw(_assets, address(this), address(this));
        ...
        if (_sender != _owner) {
            ...
            if (currentAllowance != uint256(type(uint256).max)) {
                ...
                _approve(_owner, _sender, currentAllowance - shares);
            }
        }
        ...
        emit Withdraw(_sender, _receiver, _owner, _assets, shares);
        return shares;
    }
    ...
    function maxWithdraw(address owner) public view returns (uint256) {
        return underlyingVault.maxWithdraw(owner);
    }

    function previewWithdraw(uint256 _assets) public view returns (uint256) {
        return underlyingVault.previewWithdraw(_assets);
    }

    function withdraw(uint256 _assets, address _receiver, address _owner) public nonReentrant returns (uint256) {
        return _withdraw(_assets, _receiver, _owner, msg.sender);
    }
    ...
    function redeem(uint256 _shares, address _receiver, address _owner) public nonReentrant returns (uint256) {
